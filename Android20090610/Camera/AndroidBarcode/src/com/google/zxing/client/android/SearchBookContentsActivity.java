...
public final class SearchBookContentsActivity extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		if (intent == null
				|| (!intent.getAction().equals(
						Intents.SearchBookContents.ACTION) && !intent
						.getAction().equals(
								Intents.SearchBookContents.DEPRECATED_ACTION))) {
			finish();
			return;
		}
		...
		mHeaderView = (TextView) factory.inflate(
				R.layout.search_book_contents_header, mResultListView, false);
	...
	private void handleSearchResults(JSONObject json) {
		try {
			...
			mHeaderView.setText("Found "
					+ ((count == 1) ? "1 result" : count + " results"));
			if (count > 0) {
				JSONArray results = json.getJSONArray("search_results");
				SearchBookContentsResult.setQuery(mQueryTextView.getText()
						.toString());
				...
				mResultListView.setAdapter(new SearchBookContentsAdapter(this,
						items));
			} else {
	...
	private static final class NetworkThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				URI uri = new URI("http", null, "www.google.com", -1, "/books",
						"vid=isbn" + mISBN + "&jscmd=SearchWithinVolume2&q="
								+ mQuery, null);
