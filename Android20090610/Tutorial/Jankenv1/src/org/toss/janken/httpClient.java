...
public class httpClient {
	...
	private static HttpEntity DoRequest(HttpUriRequest method) throws ClientProtocolException, IOException {
		...
		try {
			...
			if (statuscode == HttpStatus.SC_OK || statuscode == HttpStatus.SC_CREATED || statuscode == HttpStatus.SC_MOVED_PERMANENTLY){ 
//				if (response.getEntity() != null) {
//					Log.v(TAG, response.getEntity().toString());
//				}
				if (statuscode == HttpStatus.SC_MOVED_PERMANENTLY) {
					Header headers[] = response.getHeaders("Location");
					if (headers[0] != null) {
						Log.e(TAG, headers[0].toString());
