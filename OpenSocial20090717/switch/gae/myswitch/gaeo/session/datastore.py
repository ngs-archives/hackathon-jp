...
class SessionStore(db.Model):
    ...
    @classmethod
    def clear(cls):
        lst = cls.gql("WHERE expires < :1", datetime.datetime.now()).fetch(1000)
        for item in lst:
            item.delete()
...
class DatastoreSession(session.Session):
    ...
    def put(self):
        if not self._invalidated and self._store:
            ...
            self._store.expires = datetime.datetime.now() + SESSION_DURATION
            self._store.put()
    ...
    def _create_store(self, id):
        self._store = SessionStore(
                                   ...
                                   expires = datetime.datetime.now() + SESSION_DURATION)
        self._store.put()
        self._id = id
