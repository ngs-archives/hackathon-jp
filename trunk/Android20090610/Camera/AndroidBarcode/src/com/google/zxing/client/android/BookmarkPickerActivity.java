...
public final class BookmarkPickerActivity extends ListActivity {
	...
	protected void onCreate(Bundle icicle) {
		...
		mCursor = getContentResolver().query(Browser.BOOKMARKS_URI,
				BOOKMARK_PROJECTION, BOOKMARK_SELECTION, null, null);
		startManagingCursor(mCursor);

		ListAdapter adapter = new SimpleCursorAdapter(this,
				R.layout.bookmark_picker_list_item, mCursor,
	...
	protected void onListItemClick(ListView l, View view, int position, long id) {
		if (mCursor.moveToPosition(position)) {
			...
			intent.putExtra(Browser.BookmarkColumns.TITLE, mCursor
					.getString(TITLE_COLUMN));
			intent.putExtra(Browser.BookmarkColumns.URL, mCursor
