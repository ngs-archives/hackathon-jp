class PasswordsController < ApplicationController
  ...
  def create
    ...
    @password.account = Account.find_by_email(@password.email)
  ...
  def reset
    begin
      @account = Password.find(:first, :conditions => ['reset_code = ? and expiration_date > ?', params[:reset_code], Time.now]).account
    rescue
  ...
  def update_after_forgetting
    ...
    @account = @password.account unless @password.nil?
    
    respond_to do |format|
      if @account.update_attributes(params[:account])
       @password.destroy
        PasswordMailer.deliver_reset_password(@user)
        PasswordMailer.deliver_reset_password(@account)
