...
public class YahooFuriganaParser {
	private final static String YAHOO_URL = "http://jlp.yahooapis.jp/MAService/V1/parse";
	private final static String APP_ID = "PnCgp62xg67pFfuS6WLKNxe9pqPxsu3eN1TTlvOdvz0uA1Gi.oBAT.0OrU3ZtA--";
    ...
    private static void recursiveWalk(Node node){
	    ...
	    while (child!=null) {
	    	recursiveWalk(child);               // さらに子ノードをたどる
	    	try {
	    		child = child.getNextSibling();  // 兄弟ノードを取得
	    	} catch (Throwable e) {
	    		break;
	    		//ArrayIndexOutofExceptionでブレイクしないと無限ループ
	        }
	    }
    }
