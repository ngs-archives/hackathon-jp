...
public class OpenSocialActivity {
	...
	public OpenSocialActivity(Activity context,
			Map<OpenSocialProvider, Token> supportedProviders,
			String androidScheme) {
		this.context = context;
		this.prefs = context.getSharedPreferences("default",
	...
	public OpenSocialClient getOpenSocialClient() {
		...
		if (accessToken.token == null
				&& (intent.getData() == null || providerString == null)) {
			// If the user is not already authenticated and this isn't a
		...
		if (intent.getData() != null) {
			try {
				accessToken = client.getAccessToken(provider,
						loadRequestToken());
			} catch (IOException e) {
				throw new RuntimeException(
						"Error occured fetching access token", e);
		...
		client.setProperty(OpenSocialClient.Properties.ACCESS_TOKEN,
				accessToken.token);
		client.setProperty(OpenSocialClient.Properties.ACCESS_TOKEN_SECRET,
				accessToken.secret);
	...
	private void setupUsersOauthToken(
			Map<OpenSocialProvider, Token> supportedProviders,
			String browserScheme) {

		Intent oauthLibrary = new Intent(context,
				OpenSocialChooserActivity.class);

		ArrayList<String> providerStrings = new ArrayList<String>();
		for (Map.Entry<OpenSocialProvider, Token> entry : supportedProviders
				.entrySet()) {
			providerStrings.add(entry.getKey().providerName);
			addConsumerTokenExtra(oauthLibrary, entry.getKey(), entry
					.getValue());
		}

		oauthLibrary.putExtra(OpenSocialChooserActivity.ANDROID_SCHEME,
				browserScheme);
		oauthLibrary.putStringArrayListExtra(
				OpenSocialChooserActivity.PROVIDERS, providerStrings);

		// oauthLibrary.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

		context.startActivity(oauthLibrary);
	}

	private void addConsumerTokenExtra(Intent oauthLibrary,
			OpenSocialProvider provider, Token consumerToken) {
		if (consumerToken != null) {
			oauthLibrary.putExtra(provider.toString(), new String[] {
					consumerToken.token, consumerToken.secret });
		}
	}
	...
	private void persistAccessToken(Token userSpecificAccessToken) {
		...
		editor.putString(ACCESS_TOKEN_SECRET_PREF,
				userSpecificAccessToken.secret);

		editor.commit();
	}

	private Token loadAccessToken() {
		return new Token(prefs.getString(ACCESS_TOKEN_PREF, null), prefs
				.getString(ACCESS_TOKEN_SECRET_PREF, null));
	}
	...
	private Token loadRequestToken() {
		String requestTokenPref = prefs.getString(
				OpenSocialChooserActivity.REQUEST_TOKEN_PREF, null);
